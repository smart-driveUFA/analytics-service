...
from src.search_coordinates.search_end_coor import (
    find_coordinates_end_of_highway,
)
...
async def create_tpi(request: Request, tpi_data: TPI) -> JSONResponse:
    ...
    if request.headers.get("Authorization", None):
        ...
        if isinstance(coordinates_end_lon, (int, float)) and isinstance(
            ...
            if tpi_response:
                if isinstance(tpi_response, dict) and tpi_response.get(
                    "detail", None
                ):
                    return JSONResponse(
...
@router.post(
    "/traffic-status", response_model=SummingData, response_class=JSONResponse
)
async def collect_road_data(
    ...
    if request.headers.get("Authorization", None):
        ...
        if coor_end_point.get("lat_end", None) and coor_end_point.get(
            "lon_end", None
        ):
            ...
            await send_result_auth(result_process, token, route_coor)
