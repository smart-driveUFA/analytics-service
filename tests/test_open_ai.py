...
async def test_response_openai():
    mock_response__convert_data_to_message_openai = mock.AsyncMock(return_value=message_for_chatgpt)
    mock__send_request_openai_chat_completion = mock.AsyncMock(return_value=result_chatgpt)
    with mock.patch(
        ...
        result = await response_openai(response_api, start_data["lat"], start_data["lon"])
